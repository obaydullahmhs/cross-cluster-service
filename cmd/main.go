	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		Cache: cache.Options{
			ByObject: map[client.Object]cache.ByObject{
				// Secrets are cached from the credentials namespace and nowhere
				// else. This is the enforcement, not the decoration: a watch
				// predicate filters events after they arrive, so it would still
				// have required cluster-scoped list/watch on secrets -- which
				// is exactly the blanket read of every credential in the
				// cluster that the no-namespace-field design exists to prevent.
				&corev1.Secret{}: {
					Namespaces: map[string]cache.Config{credentialsNamespace: {}},
				},
			},
		},
